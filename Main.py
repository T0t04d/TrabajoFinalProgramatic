...
def p1():
    ...
    Comparativa_visitante_local = {'Locales ganados':Lganados, 'Visitantes ganados':Vganados}
    plt.pie(Comparativa_visitante_local.values(),labels=Comparativa_visitante_local.keys(),colors=colores, autopct=make_autopct([Vganados,Lganados]))
...
def p2():
    ...
    gana_masOmenos_zurdos = {'victorias mas zurdos (local)':victorias_mas_zurdos_local,'victorias menos zurdos (local)':victorias_menos_zurdos_local,'victorias mas zurdos (visitante)':victorias_mas_zurdos_visitante,'victorias menos zurdos (visitante)':victorias_menos_zurdos_visitante}
    ...
    plt.style.use('dark_background')
    plt.figure(facecolor='#37474f')
    coloritos = ["#EE6055","#60D394","#FFD97D","#FF9B85"]
    plt.pie(gana_masOmenos_zurdos.values(),labels=gana_masOmenos_zurdos.keys(),colors=coloritos,autopct=make_autopct(lista_victorias))
    plt.show()
